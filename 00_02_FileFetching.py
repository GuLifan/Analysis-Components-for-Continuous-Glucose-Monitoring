...
# 抓取需要计算的患者的CGM记录文件
# LAST UPDATE BY LIFANGU IN 202602010210
...
# -- 配置路径 ---
excel_path = r"C:\Users\lifan\Desktop\DataRequest2505EX.xlsx"
input_folder = r"C:\Users\lifan\Desktop\02_OriginalData\CGMOriginalDataAll_250531"
name_tag = "2505EX"
output_folder = fr"C:\Users\lifan\Desktop\DataSelected{name_tag}"
# 以下为匹配标识符，2024年12月之前的两个文件应该填写 7 （手机号码），2024年12月之后的一个文件应该填写 6 （探头号）
identifier_col = 6  # 匹配标识符所在列，注意文件和时间关系，不需要将第一列当作第 0 列
adm_time_col = 5  # 入院时间所在列
dis_time_col = 4  # 出院时间所在列

# 最小文件大小判定：最小有效行数（<24h则跳过）
min_valid_hours = 24  # 规则1：最小有效行数（<24h则跳过）

# 胰岛素泵时间判定：最接近胰岛素泵时间匹配
enable_pump_match = True # 是否启用规则2（默认关闭）
pump_start_col = 2  # 胰岛素泵开始时间所在列
pump_end_col = 3    # 胰岛素泵停止时间所在列
