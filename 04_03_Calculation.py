...
print("01_02_Calculation.py 启动中...", flush=True)
print(f"  工作目录: {os.getcwd()}", flush=True)

def load_config(config_path='config.yaml'):
    abs_path = os.path.abspath(config_path)
    print(f"  加载配置: {abs_path}", flush=True)
    print(f"  文件存在: {os.path.exists(config_path)}", flush=True)
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            return yaml.safe_load(f)
    except Exception as e:
        print(f"Error loading config.yaml: {e}", flush=True)
        sys.exit(1)
...
# --- requestall.csv 列名 → 标准字段映射 ---
CSV_COLUMN_MAP = {
    '住院号':       'hospital_id',
    '一次性探头编号':  'sensor_id',
    '电话号码':      'phone_number',
    '入院日期':      'admission_time',      # 住院入院时间
    '动态血糖开立时间': 'cgm_start_time',     # CGM 设备佩戴开始时间
    '出院日期':      'discharge_time',
...
def read_patient_csv(csv_path):
    """读取 requestall.csv → 映射为标准字段，过滤无效行"""
    ...
    for col in ['hospital_id', 'sensor_id', 'phone_number', 'admission_time',
                'cgm_start_time', 'discharge_time', 'pump_start_time',
                'pump_end_time', 'degu_start_time', 'ganjing_start_time']:
...
def process_patient_file(file_path, admission_time=None, discharge_time=None, pump_start_time=None, pump_end_time=None, degu_start_time=None, ganjing_start_time=None, current_mode=None):
    """
    处理患者CGM数据文件
    """
    # 使用传入的模式或全局模式
    ...
    # --- 统一 5 分钟采样 ---
    #   - 填充文件 (0 0 0 0 13.6): 剔除近零行
    #   - 1 分钟真实数据: 每 5 点保留第 1 个 (iloc[::5])
    glucose_before_clip = df['glucose'].dropna()
    near_zero_ratio = (glucose_before_clip <= 0.5).sum() / max(len(glucose_before_clip), 1)
    if near_zero_ratio > 0.7:
        df = df[df['glucose'] > 0.5].copy()
        print(f"  5 分钟填充文件，过滤后 {len(df)} 行", flush=True)
    else:
        df = df.iloc[::5].copy()
        print(f"  1 分钟文件降采样到 5 分钟，{len(df)} 行", flush=True)

    df['glucose'] = df['glucose'].clip(1.8, 33.3)

    if df.empty:
        print(f"文件 {file_path} 采样后无有效数据，跳过")
        return {'patient_id': os.path.splitext(os.path.basename(file_path))[0],
    ...
    if selected_mode == 0:  # 开始时间~开始时间+duringday天
        ...
        # 泵结束时间为空时，尝试以德谷/甘精开立时间回退（取较早者）
        if pd.isna(pump_end_time) and not pd.isna(pump_start_time):
            fallback = None
            for t in [degu_start_time, ganjing_start_time]:
                if not pd.isna(t):
                    fallback = t if fallback is None else min(fallback, t)
            if fallback is not None:
                pump_end_time = fallback
                print(f"  泵结束时间回退: {pump_end_time}")
    ...
    # Mode 7 specific note + 实际计算起止时刻
    if selected_mode == 7:
        stats['pump_start_actual'] = start_time.strftime('%Y-%m-%d %H:%M:%S') if pd.notna(start_time) else ''
        stats['pump_end_actual'] = end_time.strftime('%Y-%m-%d %H:%M:%S') if pd.notna(end_time) else ''
        if 'duration_note' in locals() and duration_note:
...
def main(selected_mode=None):
    ...
    # 读取患者列表 CSV
    try:
        patient_df = read_patient_csv(patient_list_file)
    except Exception as e:
        print(f"无法读取患者列表文件 {patient_list_file}: {e}")
        return

    # 转换时间格式
    for time_col in ['admission_time', 'cgm_start_time', 'discharge_time', 'pump_start_time', 'pump_end_time', 'degu_start_time', 'ganjing_start_time']:
    ...
    for index, row in patient_df.iterrows():
        ...
        degu_start_time = row.get('degu_start_time', None)
        ganjing_start_time = row.get('ganjing_start_time', None)
...
if __name__ == "__main__":
    ...
    if args.calc_groups is not None:
        ...
        try:
            ...
            print("Error parsing --calc_groups: " + str(e))

    mode_desc = {
        0: "模式0: 开始时间 ~ 开始时间+" + str(duringday) + "天",
        1: "模式1: 开始时间+" + str(interimday) + "天 ~ 开始时间+" + str(duringday) + "天",
        2: "模式2: 开始时间 ~ 结束时间",
        3: "模式3: 出院时间-" + str(duringday) + "天 ~ 出院时间",
        4: "模式4: 入院时间 ~ 出院时间",
        5: "模式5: 出院时间 ~ 出院时间+" + str(duringday) + "天",
        6: "模式6: 出院时间 ~ 结束时间",
        7: "模式7: 胰岛素泵使用时间"
    }

    current_mode = selected_mode if selected_mode is not None else mode
    print("当前使用" + mode_desc[current_mode], flush=True)
    print("  患者列表: " + patient_list_file, flush=True)
    print("  数据目录: " + data_folder, flush=True)
    print("  输出目录: " + output_folder, flush=True)
